...
- `ai-service.ts` — AI 服务（多提供商、evaluateOrder、streamChat、fetchCalendarEvents）
- `ai-skills.ts` — AI 技能模块（内置技能提示词内容，技能上下文注入系统提示）
- `calendar-service.ts` — 财经日历服务（AI 优先，Mock 降级）
- `fundamentals-service.ts` — 基本面数据服务
- `screener-service.ts` — 选股器服务
- `win-rate-service.ts` — 胜率分析服务
- `store.ts` — 本地配置持久化
...
| 设置 | `modules/settings/` | 基础设置 + 图表设置 + 交易设置 + 富途/老虎网关配置 + AI 提供商设置 + AI 技能配置 |
| 账户 | `modules/account/` | 账户资产详情（总资产/现金/市值/保证金/分市场明细） |
| 财经日历 | `modules/calendar/` | AI 驱动财经日历，支持日期选择、加载状态、数据来源标识 |
| 基本面 | `modules/fundamentals/` | 基本面面板（财务数据概览） |
| 胜率分析 | `modules/analytics/` | 胜率分析与绩效分解 |
| 选股器 | `modules/screener/` | 多条件选股筛选 |
| 快捷键 | `modules/shortcuts/` | 键盘快捷键浮层 |
| AI 对话 | `modules/ai/` | 对话式 AI 助手，流式输出 |
| 侧边栏 | `modules/sidebar/` | 左侧导航栏，图标 + 悬停提示 |
...
- 语言：简体中文/English，完整 i18n 集成，使用 `useT()` / `useTBatch()` / `useLocale()` 钩子，所有组件实时切换语言
...
│   │   ├── ai-service.ts      # AI 服务（多提供商、评估、对话、日历）
│   │   ├── ai-skills.ts       # AI 技能（内置技能提示词 + 上下文注入）
│   │   ├── calendar-service.ts # 财经日历（AI 优先，Mock 降级）
│   │   ├── fundamentals-service.ts # 基本面数据服务
│   │   ├── screener-service.ts # 选股器服务
│   │   ├── win-rate-service.ts # 胜率分析服务
│   │   ├── store.ts           # 配置持久化
...
│       ├── mock.ts            # Mock 数据生成
│       ├── store/
│       │   ├── index.ts       # Zustand store（slice 组合）
│       │   ├── gateway.ts     # 网关连接 slice
│       │   ├── chart.ts       # 图表数据 slice
│       │   ├── watchlist.ts   # 自选股 slice
│       │   ├── trading.ts     # 交易 slice
│       │   ├── ui.ts          # UI 状态 slice
│       │   ├── settings.ts    # 设置 slice
│       │   ├── quant.ts       # 量化/回测 slice
│       │   ├── screener.ts    # 选股器 slice
│       │   ├── alerts.ts      # 预警 slice
│       │   └── panels.ts      # 面板布局 slice
│       │   ├── en.ts          # 英文翻译（懒加载）
│       │   ├── zh.ts          # 中文翻译
│       │   └── index.ts       # i18n 钩子（useT, useTBatch, useLocale）
│       ├── utils/
│       │   ├── format.ts      # 日期时间格式化
│       │   ├── fee-calculator.ts  # 交易费用计算引擎
│       │   └── export.ts      # 数据导出工具
│       ├── components/
│       │   └── DatePicker.tsx # 共享日期选择器组件
│       ├── styles/
...
渲染进程入口已优化以加快加载速度：
- 英文翻译文件（`en.ts`）懒加载 — 仅在语言切换为英文时加载
- 设置页和快捷键浮层懒加载 — 仅在用户访问时加载
- Rspack 生产压缩和 tree-shaking 将入口体积从 ~964KB 优化至 ~332KB
- Zustand store 按模块拆分为 10 个 slice，提升代码组织和可维护性
...
用户可以启用/禁用 AI 技能，这些技能会将领域专业知识注入到 AI 提示词中。每个启用的技能会在系统消息中追加详细的知识块，让 AI 模型拥有专业上下文以提供更精准的分析。

| 技能 | 分类 | 默认 | 说明 |
|------|------|------|------|
| 技术分析 | 分析 | 开启 | MA、RSI、MACD、布林带、KDJ、OBV、成交量分析，用于订单评估 |
| 市场知识 | 市场 | 开启 | 港股/美股/A股市场规则、交易时间、结算制度、费用结构、手数 |
| 风险评估 | 风险 | 开启 | 仓位管理、回撤控制、集中度风险、VaR、流动性风险 |
| 交易策略 | 策略 | 关闭 | 动量、均值回归、突破、趋势跟踪、日内、波段交易策略 |
| 费用计算 | 费用 | 关闭 | 港股/美股/A股佣金、印花税、SEC/FINRA 费用计算及示例 |
| 形态识别 | 形态 | 关闭 | 支撑/阻力、头肩顶/底、三角形、旗形、K线形态 |

**工作原理**：

1. 技能定义位于 `src/shared/types.ts`（`AI_SKILL_PRESETS`、`DEFAULT_AI_SKILLS`）
2. 每个技能的详细提示词内容在 `src/main/ai-skills.ts`（`buildSkillPrompt()` 合并启用的内置技能 + 自定义技能）
3. `src/main/ai-service.ts` 使用 `buildSystemPrompt()` 将用户的基础系统提示与启用的技能内容合并，在每次 AI 调用前（evaluateOrder、streamChat、Claude fallback）注入
4. 设置面板（`AISettings.tsx`）提供每个技能的开关卡片，与 AI 配置一同持久化到 `ai-config.json`


#### 自定义技能

用户可以创建自己的技能，编写自定义提示词：
- 在 AI 设置的"自定义技能"区域点击 **"添加技能"**，或点击 **"导入 ZIP"** 上传技能包
- 填写名称、描述、分类和提示词内容，或从 ZIP 文件导入
- 自定义技能与内置技能一起显示，支持启用/禁用开关
- 支持编辑（铅笔图标）和删除（叉号图标）
- ZIP 包应包含 `skill.json`（或 `SKILL.md`/`prompt.txt`）文件，包含技能元数据和提示词内容

#### SkillHub 技能市场

浏览并安装社区贡献的技能：
- 在 SkillHub 区域配置自定义数据源 URL（留空使用默认地址）
- 点击 **"浏览技能"** 获取并展示可用技能
- 应用从可配置的 URL 获取技能列表（默认：`https://zhongzhong0505.github.io/bang-skillhub/registry.json`），失败时回退到本地 `skillhub/registry.json`
- 每个 SkillHub 技能显示名称、作者、描述和下载量
- 点击 **"安装"** 将技能添加到自定义技能列表
- 已安装的技能显示绿色 **"已安装"** 标记
- **来源指示器** 显示数据来自远程（绿色）还是本地回退（黄色）
- **刷新** 按钮重新从配置的 URL 获取

内置 SkillHub 技能包括：宏观经济分析、板块轮动分析、期权与希腊字母、加密市场关联、行为金融学偏差

**架构**：
- `CustomSkill` 和 `SkillHubItem` 类型定义在 `src/shared/types.ts`
- 自定义技能存储在 `ai-config.json` 的 `customSkills: CustomSkill[]` 字段
- SkillHub 通过 IPC（`skillhub:fetch`）获取，支持远程 + 本地回退
- `src/main/ai-skills.ts` 的 `buildSkillPrompt()` 同时合并内置技能和自定义技能
