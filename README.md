...
Bang 是一个基于 Electron + React + TypeScript 构建的桌面级交易终端，以 TradingView 为参照目标，集成了行情展示、技术分析、量化回测、交易下单、费用计算等全链路能力。底层通过富途 OpenD 和老虎 OpenAPI 两种券商网关获取实时行情和执行交易指令，前端使用 lightweight-charts 渲染专业 K 线图。
...
- `tiger-gateway.ts` — 老虎 OpenAPI 客户端，REST 请求 + RSA-SHA256 签名 + WebSocket 实时推送，支持 kline/quote/positions/orders/assets 方法
- `gateway-router.ts` — 网关路由器，根据 `activeProvider` 在富途和老虎之间切换
- `adapters/futu-adapter.ts` — 富途原始数据解析器，将 ProtoID 响应转换为统一内部类型
- `adapters/tiger-adapter.ts` — 老虎原始数据解析器，将 REST/WS 响应转换为统一内部类型
- `store.ts` — 本地配置持久化
...
- 网关配置：富途 OpenD（host/port/wsPort/wsAuthKey/SSL）和老虎 OpenAPI（tigerId/account/privateKey/env）
...
| REST 方法 | 功能 | 适配器函数 |
|-----------|------|-----------|
| kline | K 线数据 | `parseTigerKline` |
| quote | 实时报价 | `parseTigerSnapshot` |
| positions | 持仓列表 | `parseTigerPositions` |
| orders | 委托列表 | `parseTigerOrders` |
| assets | 账户资产 | `parseTigerAccountSummary` |
| place_order | 下单 | `buildTigerPlaceOrderBody` / `parseTigerPlaceOrder` |
| cancel_order | 撤单 | 主进程内部 |

| WebSocket 推送 | 解析函数 |
|----------------|----------|
| quote | `parseTigerWsQuote` |
| kline | `parseTigerWsKline` |
| order | 主进程转发 |
| position | 主进程转发 |
...
使用 OpenAI 的 streaming API 实现实时打字效果。主进程通过 IPC 逐 chunk 推送给渲染进程：
...
for await (const chunk of stream) {
  mainWindow.webContents.send(IPC.AI_STREAM_CHUNK, chunk.choices[0]?.delta?.content);
}
```
...
| 优先级 | 功能 | 理由 |
|--------|------|------|
| P0 | 实时 AI 行情解读 | 价值最高，实现最简单，直接复用现有指标数据 |
| P0 | AI 设置面板 | 用户需要配置 API key 和模型选择 |
| P1 | 对话式行情助手 | 交互体验最好，但实现复杂度较高 |
